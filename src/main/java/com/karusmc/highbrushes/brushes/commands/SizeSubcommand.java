 ...
 * Changes the player's brush size.
 */
public class SizeSubcommand implements Subcommand, CommandUtil {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setSize(BrushHandler.getDefaultSize());
        ...
        if (size < 0) {
            ...
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setSize(size);
