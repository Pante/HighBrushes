 ...
 * Changes the player's brush type.
 */
public class SelectSubcommand implements Subcommand, CommandUtil {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setType(BrushHandler.getDefaultBrush());
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setImage(BrushHandler.getBrushes().get(BrushHandler.getDefaultBrush()));
            
            sender.sendMessage(ChatColor.GOLD + "Brush type set to: default (" + ChatColor.RED + BrushHandler.getDefaultBrush() + ChatColor.GOLD + ")");
            
        } else if (BrushHandler.getBrushes().containsKey(args[1])) {
            
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setType(args[1]);
            PlayerHandler.PLAYERS.get(player.getUniqueId()).setImage(BrushHandler.getBrushes().get(args[1]));
            
            sender.sendMessage(ChatColor.GOLD + "Brush type set to: " + ChatColor.RED + args[1]);
