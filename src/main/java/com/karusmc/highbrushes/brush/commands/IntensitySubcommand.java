...
public class IntensitySubcommand implements Subcommand, CommandUtil {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            PlayerHandler.PLAYERS.get(player.getUniqueId()).getBrush().setSize(BrushHandler.getDefaultIntensity());
        ...
        if (intensity < 0) {
            ...
            PlayerHandler.PLAYERS.get(player.getUniqueId()).getBrush().setIntensity(intensity);
